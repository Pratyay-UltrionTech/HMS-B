...
class Medicine(Base):
    """Medicine master — stock lives on batches / inventory summary, not here as authority."""
    ...
    is_high_alert: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, index=True)
    high_alert_category: Mapped[str | None] = mapped_column(String(64), nullable=True)
    requires_dual_signoff: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
...
class MedicineBatch(Base):
    ...
    # Future barcode scanner — do not remove; search API will use these.
    barcode_value: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
...
class MedicineInventory(Base):
    """Denormalized stock summary per medicine — updated only via stock service."""
...
class StockAdjustment(Base):
    ...
    quantity: Mapped[float] = mapped_column(Float, nullable=False)  # absolute qty removed/corrected
...
class PharmacySale(Base):
    """Pharmacy counter sale — walk-in customer (name+phone) is the primary entity."""
...
class PharmacySaleItem(Base):
    ...
    purchase_price: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)  # for profit
    gst_percent: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    discount_amount: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    gst_amount: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    net_amount: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
...
class PharmacyReturn(Base):
    """Customer return or purchase return."""
...
class PharmacyRxRequest(Base):
    """Optional doctor → pharmacy handoff (structured). Counter walk-ins do not require this."""
